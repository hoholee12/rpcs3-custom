...
public:
	...
	const struct settings
	{
		...
		const QString enable_thread_scheduler   = tr("Attempt to reduce desync on low-end CPUs by forcing affinity on recompilers and shader threads.\nOnce enabled, rec/shader threads are promoted to higher priority and are pinned to specific cores in pairs.\nNormal: pair SPU + rec / RSX + shader\nExtreme: Normal + 256 each for the number of rec/shader threads");
