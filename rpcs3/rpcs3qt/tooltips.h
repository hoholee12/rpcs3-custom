...
public:
	...
	const struct settings
	{
		...
		const QString enable_thread_scheduler   = tr("Attempt to reduce desync on low-end CPUs by forcing affinity on recompilers and shader threads.\nOnce enabled, threads are promoted to higher priority and are pinned to specific cores.\nSPU: threads have the same affinity as SPU threads. Best for smooth gameplay.\nRSX: threads have the same affinity as RSX threads. Best for reducing desync.");
