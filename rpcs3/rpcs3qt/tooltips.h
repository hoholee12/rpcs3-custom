...
public:
	...
	const struct settings
	{
		...
		const QString enable_thread_scheduler   = tr("Attempt to reduce desync by recompiling faster.\nInstead of using default 1 thread, use many number of threads to compile at instant.\nOnce enabled, recompiler threads are automatically promoted to higher priority.\nMore threads equals gaming of the scheduler plus the ability to compile relevant blocks instantly.");
		const QString accurate_rsx_reservation  = tr("Enable accurate RSX reservations as well as synchronize with SPU.\nSPU synchronization can help remove graphical glitches in some games, while introducing performance impact on others.\nMotorstorm games are known to suffer. Gran Turismo games also suffer, however it is required to fix graphical glitches and freezes.\nI did make some adjustments to the scheduling to make this issue less apparent...");
