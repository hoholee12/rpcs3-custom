...
public:
	...
	const struct settings
	{
		...
		const QString enable_thread_scheduler   = tr("Attempt to reduce desync by changing the thread priority.\n'SPU focused' drops PPU/RSX thread priority down, and is the most recommended option for reducing SPU desyncs.\nIn order to reduce SPU desync while recompiling, general thread priority is set as:\nSPU recompiler(highest) > PPU/SPU/RSX(your choice to drop some down) > Shader compiler(lowest)");
