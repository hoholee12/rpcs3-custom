...
public:
	...
	const struct settings
	{
		...
		const QString spurs_urgent_queue_size      = tr("If not 0, max number of queued urgent SPURS commands per JobChain");
		...
		const QString enable_thread_scheduler   = tr("Attempt to reduce desync on low-end CPUs by forcing affinity on recompilers and shader threads.\nOnce enabled, rec/shader threads are promoted to higher priority and are pinned to specific cores.\nNormal: pair SPU + SPURecompiler / RSX + shader, Reverse: pair SPU + shader / RSX + SPURecompiler\nDifferent games require different settings. In general, SPURecompiler is run mostly in initial stage, while shader is run almost every time.");
		const QString accurate_rsx_reservation  = tr("Enable accurate RSX reservation with SPU yield.\nSPU yielding eliminates graphical glitches in some games, while affecting the performance on others.\nMotorstorm games are known to suffer, so turn it off. Gran Turismo games however, needs this to fix graphical glitches and freezes.\nI did make some adjustments to the scheduling to make the performance impact less apparent...");
		...
		const QString disable_vulkan_fence		   = tr("Disables Vulkan Fence.");
