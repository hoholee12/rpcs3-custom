...
struct music_decode
{
	...
	CellMusicSelectionContext::music_selection_context current_selection_context{};
	...
	error_code set_decode_command(s32 command)
	{
		...
		case CELL_MUSIC_DECODE_CMD_START:
		{
			...
			CellMusicSelectionContext::music_selection_context context = current_selection_context;
	...
	[&dec, dir_path, vfs_dir_path](s32 status, utils::media_info info)
	{
		sysutil_register_cb([&dec, dir_path, vfs_dir_path, info, status](ppu_thread& ppu) -> s32
		{
			...
			if (result == CELL_OK)
			{
				...
				dec.current_selection_context.create_playlist(CellMusicSelectionContext::music_selection_context::get_next_hash());
...
error_code cellMusicDecodeSetSelectionContext(vm::ptr<CellMusicSelectionContext> context)
{
	...
	if (result)
		...
		cellMusicDecode.error("cellMusicDecodeSetSelectionContext: failed. context = '%s'", CellMusicSelectionContext::music_selection_context::context_to_hex(*context));
	sysutil_register_cb([&dec, result](ppu_thread& ppu) -> s32
	{
...
error_code cellMusicDecodeSetSelectionContext2(vm::ptr<CellMusicSelectionContext> context)
{
	...
	if (result)
		...
		cellMusicDecode.error("cellMusicDecodeSetSelectionContext2: failed. context = '%s'", CellMusicSelectionContext::music_selection_context::context_to_hex(*context));
