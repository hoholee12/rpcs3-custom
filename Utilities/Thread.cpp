		...
		if (g_cfg.core.thread_scheduler != thread_scheduler_mode::none)
		{
			if (g_native_core_layout == native_core_arrangement::intel_ht)
			{
				...
				else
				{
					...
					switch (group)
					{
					case thread_class::sha: return all_cores_mask & 0b11;
					case thread_class::rsx: return all_cores_mask & 0b111; //limit to 2 physical cores
					case thread_class::rec: return all_cores_mask & (ipow(2, thread_count) - 16);
					case thread_class::spu: return all_cores_mask & (ipow(2, thread_count) - 8); //pin out one core
					case thread_class::ppu:
					default: return all_cores_mask;
					}
				}
			...
			else
			{
				...
				else
				{
					...
					switch (group)
					{
					case thread_class::sha: return all_cores_mask & 0b1;
					case thread_class::rsx: return all_cores_mask & 0b11; //limit to 2 cores
...
u64 thread_ctrl::get_recommended_workercount(thread_class group)
{
	...
	if (const auto thread_count = utils::get_thread_count())
	{
		//rsx+recompiler threads should not interfere with ppu+spu thread.
		//rsx uses 2 threads total(without multithreading).
		if (g_cfg.core.thread_scheduler == thread_scheduler_mode::normal)
		{
			if (g_native_core_layout == native_core_arrangement::intel_ht)
			{
				if (thread_count > 12)
				...
				else
				{
					...
					switch (group)
					{
					case thread_class::sha: return 2;
					case thread_class::rsx: return 3; //limit to 2 physical cores
					case thread_class::rec: return thread_count - 4;
					case thread_class::spu: return thread_count - 3; //pin out one core
					case thread_class::ppu:
					default: return thread_count;
					}
				}
			...
			else
			{
				...
				else
				{
					...
					switch (group)
					{
					case thread_class::sha: return 1;
					case thread_class::rsx: return 2; //limit to 2 cores
		...
		else if (g_cfg.core.thread_scheduler == thread_scheduler_mode::extreme)
		{
			return 256;
		}
		else
